...
const schema = z.object({ reason: z.string().trim().min(10).max(1000), idempotencyKey: z.string().uuid() });
export async function POST(request: Request, context: { params: Promise<{ proposalId: string }> }) {
  ...
  if (!isUuid(proposalId) || !parsed.success) return proposalFailure(422, "VALIDATION_ERROR", "Lý do huỷ đề xuất chưa hợp lệ.", id);
  ...
  const { data, error } = await supabase.rpc("request_proposal_cancellation_v2", { p_proposal_id: proposalId, p_reason: parsed.data.reason, p_idempotency_key: parsed.data.idempotencyKey });
