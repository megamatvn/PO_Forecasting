...
with receipt_by_month as (
  ...
  group by purchase_batches.plan_version_id, purchase_lines.product_id,
    date_trunc('month', purchase_batches.eta_date)::date
),
projection_months as (
  select plan_monthly_demand.plan_line_id, plan_monthly_demand.demand_month as projection_month
  from public.plan_monthly_demand
  union
  select plan_lines.id, receipt_by_month.projection_month
...
line_targets as (
  ...
  group by plan_lines.id, plan_lines.target_stock,
    planning_settings.safety_stock, planning_settings.target_cover_months
),
monthly_activity as (
  select
  ...
  join public.plan_lines on plan_lines.id = projection_months.plan_line_id
  join public.plan_versions on plan_versions.id = plan_lines.plan_version_id
  join public.planning_cycles on planning_cycles.id = plan_versions.planning_cycle_id
  join line_targets on line_targets.plan_line_id = plan_lines.id
  left join public.plan_monthly_demand
    on plan_monthly_demand.plan_line_id = plan_lines.id
...
running_projection as (
  select
    brand_id, plan_version_id, plan_line_id, product_id, projection_month,
    initial_stock, target_stock, demand_qty, receipt_qty,
    initial_stock + coalesce(
      sum(receipt_qty - demand_qty) over (
        partition by plan_line_id order by projection_month
